...
async function updatePortfolioSchema() {
  try {
    ...
    // Update each portfolio with new detailed data
    for (const portfolio of portfolios) {
      const updates: any = {};

      // Add challenges based on category
      ...
      if (!portfolio.warrantyPeriod) updates.warrantyPeriod = '2 yıl';
      if (!portfolio.teamSize) updates.teamSize = '2 Kişi';
      if (!portfolio.customerRating) updates.customerRating = 5;
      if (!portfolio.difficulty) updates.difficulty = 'Orta';

      // Update the portfolio
      await Portfolio.findByIdAndUpdate(portfolio._id, updates);
